...
<head>
  ...
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.0/js/bootstrap.min.js"></script>
	<link rel="stylesheet" type="text/css" href="../style/sbpassword.css">
...
<!----------------------------subadmin change password php code-------------------->
<?php 
...
if(isset($_POST['submit']))
{
	...
	$sel_pass="SELECT * FROM admin_add_manger WHERE email='$email' AND password ='$current_password'";
	$run_pass=mysqli_query($conn,$sel_pass);
	$check_pass=mysqli_num_rows($run_pass);
	if($check_pass==0)
	{
		echo "<script> alert('Your Current Password is Wrong!')</script>";
		echo "<script>window.open('subadmin_change_password.php','_self')</script>";
	}	
	...
	{
		$update_pass="UPDATE admin_add_manger SET password='$new_password' WHERE email='$email'";
