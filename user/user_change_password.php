...
<head>
	...
	<link rel="stylesheet" type="text/css" href="../style/upassword.css">	
</head>
<body>
	<!----------------------------------creation navbar-------------------------------------->
	...
	<div class="row">
		<div class="col-md-6" id="form">
			...
			<div class="form-group">
				...
				<input type="Password" name="current_password" class="form-control text" placeholder="Enter Current Password" required="">
			</div>
			<div class="form-group">
				<label>Enter New Password:</label>
				<input type="Password" name="new_password" class="form-control text" placeholder="Enter New Password" required="">
			</div>
			<div class="form-group">
				<label>Confirm New Password:</label>
				<input type="Password" name="confirm_password" class="form-control text" placeholder="Enter Confirm New Password" required="">
			</div>
			<div class="form-group">
...
if(isset($_POST['submit']))
{
	...
	$sel_pass="SELECT * FROM user_registration WHERE email='$email' AND password ='$current_password'";
	$run_pass=mysqli_query($conn,$sel_pass);
	$check_pass=mysqli_num_rows($run_pass);
	if($check_pass==0)
	{
		echo "<script> alert('Your Current Password is Wrong!')</script>";
		echo "<script>window.open('user_change_password.php','_self')</script>";
	}	
	...
	{
		$update_pass="UPDATE user_registration SET password='$new_password' WHERE email='$email'";
